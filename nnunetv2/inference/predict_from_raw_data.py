...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            print(f'[DEBUG] 正在加载检查点文件: {join(model_training_output_dir, f"fold_{f}", checkpoint_name)}')
            checkpoint = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name),
                                    map_location=torch.device('cpu'),
                                    weights_only=False)
            if i == 0:
                trainer_name = checkpoint['trainer_name']
                print(f'[DEBUG] 检查点中的训练器名称: {trainer_name}')
        ...
        print(f'[DEBUG] 正在查找训练器类: {trainer_name}')
        trainer_class = recursive_find_python_class(join(nnunetv2.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'nnunetv2.training.nnUNetTrainer')
        if trainer_class is None:
            print(f'[DEBUG] 错误：未能找到训练器类 {trainer_name}')
            raise RuntimeError(f'Unable to locate trainer class {trainer_name} in nnunetv2.training.nnUNetTrainer. '
                               f'Please place it there (in any .py file)!')
        print(f'[DEBUG] 成功找到训练器类: {trainer_class.__name__}')
        
        print(f'[DEBUG] 开始构建网络架构...')
        ...
        print(f'[DEBUG] 网络架构构建完成: {type(network).__name__}')
        ...
        print(f'[DEBUG] 正在加载网络权重...')
        network.load_state_dict(parameters[0])
        print(f'[DEBUG] 网络权重加载完成')
