...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        ### 修改后的超参数（关键修改点）
        self.initial_lr = 0.001 # AdamW需要更小的初始学习率（原为5e-3）
        self.weight_decay = 1e-4  # 权重衰减调整（原为3e-5）
        self.oversample_foreground_percent = 0.6  # 保持原有配置
        ...
        if self.label_manager.has_regions:
            ...
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                                    ce_kwargs={'weight': torch.tensor([1.0, 5.0, 5.0, 5.0,10.0], device=self.device)}, weight_ce=1, weight_dice=1, 
...
# 增加权重调整！！
    ...
    def get_training_transforms(
        ...
        transforms.append(
            SpatialTransform(
                patch_size_spatial, patch_center_dist_from_border=0, random_crop=False, p_elastic_deform=0.5,
                p_rotation=0.2,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # Call lr_scheduler.step() after optimizer.step() without epoch parameter
